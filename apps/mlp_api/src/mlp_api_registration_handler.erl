%% basic handler
-module(mlp_api_registration_handler).
...
-record(state, {
    operation_id :: mlp_api_api:operation_id(),
    logic_handler :: atom(),
...
-spec init(Req :: cowboy_req:req(), Opts :: mlp_api_router:init_opts()) ->
...
handle_request_json(
    ...
    case mlp_api_api:populate_request(OperationID, Req0, ValidatorState) of
        {ok, Populated, Req1} ->
            {Code, Headers, Body} = mlp_api_logic_handler:handle_request(
                LogicHandler,
                OperationID,
                Req1,
                maps:merge(State#state.context, Populated)
            ),
            _ = mlp_api_api:validate_response(
                OperationID,
                Code,
                Body,
                ValidatorState
