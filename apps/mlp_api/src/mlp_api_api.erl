...
validate(Rule = {type, 'float'}, Name, Value, _ValidatorState) ->
    try
        {ok, mlp_api_utils:to_float(Value)}
    catch
        error:badarg ->
            validation_error(Rule, Name)
    end;
...
validate(Rule = schema, Name, Value, ValidatorState) ->
    Definition =  list_to_binary("#/components/schemas/" ++ mlp_api_utils:to_list(Name)),
    try
...
get_value(qs_val, Name, Req) ->
    ...
    Value = mlp_api_utils:get_opt(mlp_api_utils:to_qs(Name), QS),
    {Value, Req};

get_value(header, Name, Req) ->
    Headers = cowboy_req:headers(Req),
    Value =  maps:get(mlp_api_utils:to_header(Name), Headers, undefined),
    {Value, Req};

get_value(binding, Name, Req) ->
    Value = cowboy_req:binding(mlp_api_utils:to_binding(Name), Req),
    {Value, Req}.
...
binary_to_lower(V) when is_binary(V) ->
    list_to_binary(string:to_lower(mlp_api_utils:to_list(V))).
